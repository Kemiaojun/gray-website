<template>
  ...
  >
    <el-form
      ...
      :rules="rules"
    >
      <ImageUpload :size=150 :image-url="form.coverUrl" @select-file="onFileChange"></ImageUpload>
      ...
      <el-form-item label="描述：" prop="remark">
        <el-input type="textarea" :rows="2" v-model="form.remark" placeholder="请输入网址描述"></el-input>
      </el-form-item>
...
const handleConfirm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      ...
      formData.append('entity', JSON.stringify({id:form.id,name:form.name,remark:form.remark,url:form.url,cover:form.cover,groupType:form.groupType}));

      await addLinkApi(formData).then((rsp) => {
